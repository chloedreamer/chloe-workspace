...
export default function TaskDetailPanel({ taskId, onClose, onUpdate }: Props) {
  ...
  const updateField = async (data: Record<string, unknown>) => {
    await fetch(`/api/tasks/${task.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
    fetchTask(); onUpdate();
  };
  ...
  const addSubtask = async () => {
    ...
    setNewSubtask(""); fetchTask(); onUpdate();
  };
  ...
  const addComment = async () => {
    ...
    await fetch(`/api/tasks/${task.id}/comments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: newComment }) });
    setNewComment(""); fetchTask();
  };

  const deleteComment = async (id: string) => { await fetch(`/api/comments/${id}`, { method: "DELETE" }); fetchTask(); };

  const deleteTask = async () => { await fetch(`/api/tasks/${task.id}`, { method: "DELETE" }); onClose(); onUpdate(); };
      ...
      <div className="relative w-full max-w-lg bg-white shadow-xl flex flex-col h-full animate-slide-in">
        <div className="flex items-center justify-between p-4 border-b border-rose-border">
          ...
          <div className="flex items-center gap-2">
            <button onClick={deleteTask} className="p-1.5 rounded-lg text-rose-muted hover:text-red-500 hover:bg-red-50 transition"><Trash2 className="w-4 h-4" /></button>
            <button onClick={onClose} className="p-1.5 rounded-lg text-rose-muted hover:text-rose-deep hover:bg-rose-light transition"><X className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {editingTitle ? (
            ...
            <h2 className="text-xl font-bold text-rose-dark cursor-pointer hover:text-rose-deep" onClick={() => setEditingTitle(true)}>{task.title}</h2>
          )}

          {editingDesc ? (
            <textarea autoFocus value={task.description || ""} onChange={(e) => setTask({ ...task, description: e.target.value })}
              onBlur={() => { updateField({ description: task.description }); setEditingDesc(false); }}
              placeholder="Add a description..." className="w-full text-sm text-rose-dark border border-rose-border rounded-lg p-3 outline-none focus:ring-2 focus:ring-rose resize-none h-24" />
          ) : (
              ...
              {task.comments.map((c) => (
                <div key={c.id} className="bg-rose-light rounded-lg p-3 group">
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => deleteComment(c.id)} className="opacity-0 group-hover:opacity-100 text-rose-muted hover:text-red-400 transition flex-shrink-0 ml-2"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                  <p className="text-xs text-rose-muted mt-1">{new Date(c.createdAt).toLocaleString()}</p>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-2 mt-3">
              <input type="text" value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addComment(); }} placeholder="Add a comment..." className="flex-1 text-sm border border-rose-border rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-rose" />
              <button onClick={addComment} className="p-1.5 rounded-lg bg-rose-deep text-white hover:opacity-90 transition"><Send className="w-4 h-4" /></button>
            </div>
          </div>
        </div>

        <div className="px-5 py-3 border-t border-rose-border text-xs text-rose-muted">Created {new Date(task.createdAt).toLocaleDateString()}</div>
      </div>
    </div>
  );
}
